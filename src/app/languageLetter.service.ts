...
export class LanguageLetterService {

  private languageLettersUrl = 'http://localhost:50473/api/letters';  // URL to web api
  ...
  getLanguageLetters(languageId: number): Observable<LanguageLetter[]> {
    const url = `${this.languageLettersUrl}?languageId=${languageId}`;
    return this.http.get<LanguageLetter[]>(url)
      .pipe(
        tap(_ => this.log('fetched languageLetters')),
        catchError(this.handleError<LanguageLetter[]>('getLanguageLetters', []))
      );
  }
